...
class PerfilController extends Controller
{
    ...
    {
        return view('perfil.index'); // Asegúrate de tener esta vista
    }
    ...
    public function editar(Request $request)
    {
        ...
        return view('perfil.editar-perfil', compact('atributo'));
    }
    ...
    // Vista para ingresar saldo
    public function ingresarSaldo()
    {
        return view('perfil.ingresar-saldo'); // Vista para ingresar saldo
    }
    ...
    public function confirmarSaldo(Request $request)
    {
        ...
        if (!in_array($cantidad, [10, 50, 100])) {
            return back()->with('error', 'Cantidad no válida.');
        }

        $id = session('ID_Usuario'); // Cambia 'id' si usaste otro nombre de clave
        ...
        $nuevoSaldo = ($usuario->Saldo ?? 0) + $cantidad;
        ...
        return redirect()->route('perfil.index')->with('success', 'Saldo actualizado correctamente.');
    }
    ...
    {
        return view('perfil.cambiar-contrasena'); // Vista para cambiar contraseña
    }
